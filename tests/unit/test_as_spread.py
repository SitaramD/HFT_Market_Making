...
# These mirror what the engine must compute.
# Tests validate both the formula logic AND engine output equivalence.

def reservation_price(mid: float, q: float, sigma: float,
                      gamma: float, T: float) -> float:
    """
    r = mid - q * gamma * sigma^2 * T
    q:     current inventory in BTC (signed: + long, - short)
...
def as_spread(sigma: float, gamma: float, T: float, kappa: float) -> float:
    """
    delta = gamma * sigma^2 * T + (2/gamma) * ln(1 + gamma/kappa)
    Full Avellaneda-Stoikov optimal spread.
    """
...
def bid_price(r: float, delta: float) -> float:
    return r - delta / 2.0


def ask_price(r: float, delta: float) -> float:
    return r + delta / 2.0
...
class TestReservationPrice:
    ...
    def test_zero_inventory_equals_mid(self):
        """With no position, reservation price must equal mid."""
        r = reservation_price(MID, q=0.0, sigma=SIGMA,
                              gamma=AS_GAMMA, T=T_REMAINING)
        assert r == pytest.approx(MID, rel=1e-9)

    def test_long_inventory_skews_below_mid(self):
        """Long position → reservation price below mid (offload bias)."""
        r = reservation_price(MID, q=0.005, sigma=SIGMA,
                              gamma=AS_GAMMA, T=T_REMAINING)
        assert r < MID

    def test_short_inventory_skews_above_mid(self):
        """Short position → reservation price above mid (cover bias)."""
        r = reservation_price(MID, q=-0.005, sigma=SIGMA,
                              gamma=AS_GAMMA, T=T_REMAINING)
        assert r > MID

    def test_max_long_inventory_skew_magnitude(self):
        """At max long rail, skew must be bounded and not catastrophic."""
        r = reservation_price(MID, q=MAX_INV_BTC, sigma=SIGMA,
                              gamma=AS_GAMMA, T=T_REMAINING)
        skew = MID - r
    ...
    def test_max_short_inventory_skew_magnitude(self):
        """At max short rail, skew direction is reversed."""
        r = reservation_price(MID, q=-MAX_INV_BTC, sigma=SIGMA,
                              gamma=AS_GAMMA, T=T_REMAINING)
        skew = r - MID
        expected_skew = AS_GAMMA * (SIGMA ** 2) * T_REMAINING * MAX_INV_BTC
    ...
    def test_higher_vol_produces_larger_skew(self):
        """Higher volatility → larger inventory skew."""
        q = 0.005
    ...
    def test_zero_vol_zero_skew(self):
        """Zero volatility → zero skew regardless of inventory."""
        r = reservation_price(MID, q=0.005, sigma=0.0,
                              gamma=AS_GAMMA, T=T_REMAINING)
        assert r == pytest.approx(MID, rel=1e-9)
    ...
    def test_skew_linear_in_inventory(self):
        ...
        skews = [
            MID - reservation_price(MID, q=q, sigma=SIGMA,
                                    gamma=AS_GAMMA, T=T_REMAINING)
            for q in q_values
        ]
    ...
    def test_property_reservation_price_finite(self, q, sigma):
        """Reservation price must always be finite and positive."""
        r = reservation_price(MID, q=q, sigma=sigma,
                              gamma=AS_GAMMA, T=T_REMAINING)
    ...
    def test_property_sign_of_skew_matches_inventory_sign(self, q, sigma):
        """Sign of reservation-price skew must always oppose sign of inventory."""
        assume(abs(q) > 1e-9)
        r    = reservation_price(MID, q=q, sigma=sigma,
                                 gamma=AS_GAMMA, T=T_REMAINING)
        skew = r - MID   # positive when r > mid (short inventory)
        # skew sign should match -sign(q)
...
class TestASSpread:
    ...
    def test_spread_positive(self):
        """Optimal spread must always be positive."""
        delta = as_spread(SIGMA, AS_GAMMA, T_REMAINING, AS_KAPPA)
        assert delta > 0

    def test_spread_formula_value(self):
        """Verify exact numeric value against hand-calculated reference."""
    ...
    def test_spread_decreases_with_kappa(self):
        """Higher order arrival rate → narrower spread."""
    ...
    def test_bid_always_below_ask(self):
        """Bid must always be strictly below ask — no inverted quotes."""
        r     = reservation_price(MID, q=0.0, sigma=SIGMA,
                                  gamma=AS_GAMMA, T=T_REMAINING)
        delta = as_spread(SIGMA, AS_GAMMA, T_REMAINING, AS_KAPPA)
    ...
    def test_bid_ask_symmetric_around_reservation(self):
        """Bid and ask must be equidistant from reservation price."""
        r     = reservation_price(MID, q=0.0, sigma=SIGMA,
                                  gamma=AS_GAMMA, T=T_REMAINING)
        delta = as_spread(SIGMA, AS_GAMMA, T_REMAINING, AS_KAPPA)
    ...
    def test_no_double_vol_penalty(self):
        """
        AS model already accounts for vol via sigma^2 in reservation price.
        A vol_mult multiplier applied ON TOP would double-count risk and
        inflate spreads beyond fill tolerance. Verify spread does NOT
        contain a vol_mult factor by checking it equals the pure formula.
        """
        delta_pure    = as_spread(SIGMA, AS_GAMMA, T_REMAINING, AS_KAPPA)
        vol_mult      = 1.5   # simulated bad code applying extra multiplier
        delta_doubled = delta_pure * vol_mult
        # If engine applies vol_mult, quotes will be this much wider — BAD
        # This test documents the invariant: spread = pure AS formula only
        assert delta_pure < delta_doubled   # sanity
        # Spread must be < 0.1% of mid to be fillable in practice
    ...
    def test_spread_not_inverted_under_high_inventory(self):
        """Even at max inventory, quotes must never be inverted."""
    ...
    def test_property_spread_always_positive(self, sigma, gamma, kappa):
        """Spread must be positive for any valid parameter combination."""
    ...
    def test_property_bid_always_below_ask(self, q, sigma):
        """Bid < ask must hold for all inventory and vol combinations."""
        r     = reservation_price(MID, q=q, sigma=sigma,
                                  gamma=AS_GAMMA, T=T_REMAINING)
        delta = as_spread(sigma, AS_GAMMA, T_REMAINING, AS_KAPPA)
        assert bid_price(r, delta) < ask_price(r, delta)
