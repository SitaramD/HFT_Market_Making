...
class TestSharpeCalculation:
    ...
    def test_positive_consistent_returns_high_sharpe(self):
        """Consistent positive PnL with tiny noise must yield non-zero Sharpe."""
        rng = np.random.default_rng(99)
        pnl = list(rng.normal(10.0, 0.1, 300))  # high mean, tiny std → high Sharpe
        s   = compute_sharpe(pnl)
        assert s > 0.0

    def test_mixed_returns_lower_sharpe(self):
        """High variance PnL reduces Sharpe vs low variance."""
        rng  = np.random.default_rng(0)
        # High variance series
        pnl_noisy  = list(rng.normal(5.0, 50.0, 500))
        # Low variance series with same mean
        pnl_stable = list(rng.normal(5.0, 0.1, 500))
        s_noisy    = compute_sharpe(pnl_noisy)
        s_stable   = compute_sharpe(pnl_stable)
        assert s_noisy < s_stable
    ...
    def test_sharpe_above_target_passes_gate(self):
        """High Sharpe series must pass the gate."""
        rng = np.random.default_rng(99)
        pnl = list(rng.normal(10.0, 0.1, 300))  # high mean, tiny std
        s   = compute_sharpe(pnl)
    ...
    @given(scale=st.floats(min_value=1.0, max_value=100.0))
